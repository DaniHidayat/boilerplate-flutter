  ...
  Future _getData() async {
    try {
      final response = await http.get(Uri.parse(
          ...
          BaseUrl().getip() + "/notes/detail/${widget.id}"));
      print('inidata,${response.statusCode}');
  ...
  Future<void> _onUpdate(context) async {
    try {
      final response = await http.put(
        Uri.parse(BaseUrl().getip() + "/notes/edit/${widget.id}"),
        body: {
      ...
      if (response.statusCode == 200) {
        ...
        print(data["message"]);
        ToastCustom.toastSuccess("Berhasil di Update");
  ...
  Future _onDelete(context) async {
    try {
      return await http.delete(
        Uri.parse(BaseUrl().getip() + "/notes/delete/${widget.id}"),
        body: {
          "id": widget.id,
        },
      ).then((value) {
        ...
        print(data["message"]);
        ToastCustom.toastSuccess("Berhasil di Hapus");
        // Remove all existing routes until the home.dart, then rebuild Home.
        Navigator.of(context)
            .pushNamedAndRemoveUntil('/note', (Route<dynamic> route) => false);
      });
    } catch (e) {
      print(e);
    }
